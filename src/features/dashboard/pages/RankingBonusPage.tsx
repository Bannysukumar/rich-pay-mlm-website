...
export function RankingBonusPage() {
  const { firebaseUid, profile } = useAuthState()
  const [rows, setRows] = useState<Row[]>([])
  const [loading, setLoading] = useState(true)

  const total = profile?.rankCommissionTotal ?? 0
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, COLLECTIONS.rankBonuses),
    ...
    const unsub = onSnapshot(
      ...
      () => {
        setLoading(false)
        toast.error('Could not load ranking bonus history')
        setRows([])
      },
    )
    return () => unsub()
          ...
          <div className="col-12">
            <div className="card">
              <div className="card-header">
                <h4 className="card-title">Total Rank Commission - $ {total.toFixed(2)}</h4>
              </div>
