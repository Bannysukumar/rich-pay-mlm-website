/**
 * Reverse duplicate daily ROI + team-level payouts for one IST calendar day.
 * When `processDailyRoi` ran twice, keeps the earliest ledger row per package/team key
 * and reverses + deletes the later duplicates.
 *
 * Usage:
 *   node scripts/reverse-duplicate-daily-payouts.mjs <service-account.json>
 *   node scripts/reverse-duplicate-daily-payouts.mjs <service-account.json> --date=2026-06-16
 *   node scripts/reverse-duplicate-daily-payouts.mjs <service-account.json> --execute
 */
...
if (!keyPath || process.argv.includes('-h')) {
  console.error(
    'Usage: node scripts/reverse-duplicate-daily-payouts.mjs <service-account.json> [--date=YYYY-MM-DD] [--execute]',
  )
  process.exit(1)
}
...
/** Keep earliest row per key; return later rows as duplicates. */
function pickDuplicateRows(rows, keyFn) {
  ...
  for (const group of groups.values()) {
    ...
    group.sort((a, b) => a.ms - b.ms || a.id.localeCompare(b.id))
...
async function main() {
  ...
  const dupDaily = pickDuplicateRows(daily, (r) => r.activePackageId || r.id)
  const dupTeam = pickDuplicateRows(team, (r) => `${r.userId}|${r.fromUserId}|${r.activePackageId}|${r.level}|${r.amount}`)

  if (dupDaily.length === 0 && dupTeam.length === 0) {
    console.log('No duplicate payout rows found for this IST day.')
    return
  }
  ...
  const summary = {
    ...
    totalTeamRows: team.length,
    duplicateDailyRows: dupDaily.length,
    duplicateTeamRows: dupTeam.length,
    duplicateDailyAmount: dailyTotal,
    duplicateTeamAmount: teamTotal,
    usersToAdjust: userDebit.size,
    activePackagesToAdjust: pkgDebit.size,
    sampleDuplicateDaily: dupDaily.slice(0, 3),
    sampleDuplicateTeam: dupTeam.slice(0, 3),
  }
  console.log('\nPlanned duplicate reversal:\n', JSON.stringify(summary, null, 2))

  if (!EXECUTE) {
    console.log('\nDry run only. Re-run with --execute to apply debits and delete duplicate ledger rows.')
    return
  }
  ...
  batch = db.batch()
  ops = 0
  for (const row of [...dupDaily, ...dupTeam]) {
  ...
  console.log('\nDone. Reversed duplicate wallets/packages and deleted duplicate ledger rows for IST day', dayKey)
}
